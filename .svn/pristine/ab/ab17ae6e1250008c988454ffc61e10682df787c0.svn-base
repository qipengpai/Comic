...
public class DistributorCartoonModelController {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = ActionUrl.DISTRIBUTOR_ADD_DISTRIBUTION_CARTOONCOVER, method = RequestMethod.POST)
	public Model addDistributorTitle(DistributorCartoonModelData distributorCartoonModelData)
		...
		if (ParaClick.clickString(distributorCartoonModelData.getModelUrl())) {
			return new Model(500, "请输入文案封面");
		}
		String[] pp = distributorCartoonModelData.getModelUrl().split("\\|");
		for (int i = 0; i < pp.length; i++) {
		//  增加漫画推广文案封面
			boolean flag = distributorCartoonModelService.addDistributorModel(distributorCartoonModelData.getCartoonId(),pp[i]);
			if (!flag) {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = ActionUrl.DISTRIBUTOR_UPDATE_DISTRIBUTION_CARTOONCOVER, method = RequestMethod.POST)
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = ActionUrl.DISTRIBUTOR_DELETE_DISTRIBUTION_CARTOONCOVER, method = RequestMethod.POST)
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = ActionUrl.DISTRIBUTOR_SELECT_DISTRIBUTION_CARTOONCOVER_BYID, method = RequestMethod.POST)
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = ActionUrl.DISTRIBUTOR_SELECT_DISTRIBUTION_CARTOONCOVER, method = RequestMethod.POST)
	public Model selectDistributorTitle(DistributorCartoonModelData distributorCartoonModelData)
		...
		//  查看漫画推广文案封面数量
		int num =distributorCartoonModelService.selectDistributorModelCount(distributorCartoonModelData.getCartoonId());
		if (num==0) {
			return new Model(500, "查询失败 ");
		}
		//  查看漫画推广文案封面列表
		List<DistributorCartoonModel> list = distributorCartoonModelService.selectDistributorModel(distributorCartoonModelData.getNowPage(),distributorCartoonModelData.getPageNum(),distributorCartoonModelData.getCartoonId());
