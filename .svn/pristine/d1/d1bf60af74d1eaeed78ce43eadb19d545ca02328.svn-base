...
public class CartoonServiceImpl extends BaseServiceImpl<Cartoon> implements
	...
	public List<Cartoon> getALLCartoon(String cartoonType, String firsrtType,
		...
		sb.append(" GROUP BY c.id ORDER BY c.playCount DESC  LIMIT "
				+ (Integer.parseInt(nowPage) - 1) * 12 + ",12");
		// ///////redis
		List<Cartoon> list = super.SQL("ComicAllSecond" + cartoonType + nowPage,
	...
	@Override
	public int getFreeCartoonMoreCount() {
		// 查看所有免费漫画数量
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT  COUNT(c.id) FROM  Cartoon as c,CartoonAllModel as cam,CartoonModel as cm WHERE c.id=cam.CartoonId AND cm.id=cam.CartoonModelId  AND c.state=1 and cm.model LIKE '%免费%'");
		
		return gettotalpage("ComicFreeCount-", 3600, sb.toString());
	}

	@Override
	public List<Cartoon> getFreeCartoonMore(String nowPage) {
		// 查看所有免费漫画
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT  c.* FROM  Cartoon as c,CartoonAllModel as cam,CartoonModel as cm WHERE c.id=cam.CartoonId AND cm.id=cam.CartoonModelId  AND c.state=1 and cm.model LIKE '%免费%' ORDER BY cam.sort");
		sb.append(" LIMIT " + (Integer.parseInt(nowPage) - 1) * 10 + ",10");
		return super.SQL("ComicFree-" +nowPage, 3600,
				sb.toString(), Cartoon.class);
	}

	@Override
	public List<Cartoon> getCartoonMore(String cartoonModelId, String nowPage) {
		// 查看该模块更多漫画
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT c.* FROM Cartoon as c,CartoonAllModel as cam WHERE cam.CartoonId=c.id AND cartoonModelId='"+cartoonModelId+"' ORDER BY c.playCount DESC");
		sb.append(" LIMIT " + (Integer.parseInt(nowPage) - 1) * 10 + ",10");
		return super.SQL("ComicAllModelMoreTh-"+cartoonModelId+nowPage , 3600,
				sb.toString(), Cartoon.class);
	}
	...
	public int getCartoonMoreCount(String cartoonModelId) {
		// 查看该模块更多漫画數量
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT COUNT(c.id) FROM Cartoon as c,CartoonAllModel as cam WHERE cam.CartoonId=c.id AND cartoonModelId='"+cartoonModelId+"'");
		return super.gettotalpage("ComicAllModelMoreCount-"+cartoonModelId , 3600,
				sb.toString());
	}
