...
//			@CacheEvict(value = "CartoonLike", allEntries = true) })
	...
	public List<Cartoon> selectAllCartoon(CartoonVo cartoonData) {
		...
		sb.append(" order by c.state DESC,c.hot DESC,c.sort DESC LIMIT "
...
//	@CacheEvict(value = "Cartoon", allEntries = true)
	...
	@Override
	public List<Cartoon> selectAllCartoonByDistributor(CartoonData cartoonData) {
		// 查询分销商可见漫画
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT * FROM Cartoon  WHERE 1=1 AND distributorState=1");
		if (!ParaClick.clickString(cartoonData.getCondition())) {
			sb.append(" AND ((cartoonName LIKE '%"
					+ cartoonData.getCondition().trim()
					+ "%') OR (cartoonAuthor LIKE '%"
					+ cartoonData.getCondition().trim()
					+ "%'))");
		}
		sb.append(" ORDER BY recommendationIndex DESC LIMIT "
				+ (Integer.parseInt(cartoonData.getNowPage()) - 1)
				* Integer.parseInt(cartoonData.getPageNum())
				+ ","
				+ Integer.parseInt(cartoonData.getPageNum()));
	...
	@Override
	public int getCountByDistributor(CartoonData cartoonData) {
		// 查询分销商可见漫画数量
		StringBuffer sb = new StringBuffer();
		sb.append("select count(*) from Cartoon  where 1=1 AND distributorState=1");
		if (!ParaClick.clickString(cartoonData.getCondition())) {
			sb.append(" AND ((c.cartoonName LIKE '%"
					+ cartoonData.getCondition().trim()
					+ "%') OR (c.cartoonAuthor LIKE '%"
