...
public class CartoonTaskController {
	...
	public Model pp(UserEntityData entity) throws Exception {
		...
		if (!ParaClick.clickList(userTask)) {
			...
			// 增加第八个任务
			if (userTask.size() == 7) {
				boolean flag = userTaskService.addUserFollowTask(userEntity
						.getUserId());
				if (!flag) {
					return new Model(500, "增加失敗");
				}
	...
	public Model appInsertTask(UserEntityData entity) throws Exception {
		...
		// 检查已完成任务
		boolean flag = userTaskService.finishTask(userEntity.getUserId(),
	 ...
	 * @throws Exception
	 * */
	// INSERT INTO `CartoonTask` VALUES (1, '2017-12-25 00:00:01', '1', '1',
	// '10', '签到', '签到', '4');
	@ResponseBody
	@RequestMapping(value = ActionUrl.TEXT, method = RequestMethod.GET)
	public Model bsd(CartoonTaskData CartoonTaskData) throws Exception {
		// 获取登录用户信息
	 ...
	 * pp
	 * 
	 * @Remarks h5>个人中心>完成关注平台任务
	 * @throws Exception
	 * */
	@ResponseBody
	@RequestMapping(value = ActionUrl.FINISH_FOLLOW_TASK, method = RequestMethod.POST)
	public Model bA(CartoonTaskData CartoonTaskData) throws Exception {
		...
		// 获取登录用户信息 分享平台
		boolean flag = userTaskService.finallyFollow(CartoonTaskData.getId());
		if (!flag) {
			return new Model(500, "失败");
		}
		return new Model(200, "成功");
