...
//				}
				if (!ParaClick.clickString(newsData.getState())) {
					...
					if(Integer.parseInt(newsData.getState())==1){
						news.setSendDate(DateUtil.getdate());
					}
				}
	...
	public List<News> selectNews(NewsData newsData) {
		...
		if (!ParaClick.clickString(newsData.getContent())) {
			sb.append(" AND ((content like '%"+ newsData.getContent()+"%') or"
							+ "(title like '%"+ newsData.getContent()+"%')  or"
							+ "(type like '%"+  newsData.getContent()+"%'))");
		}
		if(!ParaClick.clickString(newsData.getState())){
