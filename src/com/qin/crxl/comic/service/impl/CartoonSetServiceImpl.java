...
//	@Cacheable(value = "CartoonSet",key = "#cartoonData.id.concat(#cartoonData.nowPage).concat(#cartoonData.mode)")
	...
	//	List<CartoonSet> list=super.SQL("Set-"+cartoonData.getId()+cartoonData.getMode()+cartoonData.getNowPage(),60,sb.toString(),CartoonSet.class);
		List<CartoonSet> list=super.SQL("Set-"+cartoonData.getId()+cartoonData.getMode(),60,sb.toString(),CartoonSet.class);
		return list;
	}
...
//	@Cacheable(value = "CartoonSet",key = "#root.methodName.concat(#cartoonData.id)")
	...
	@Override
	public boolean addPlayCount3(String cartoonSetId) {
		// 增加每话播放量
		boolean flag=false;
		try {
			CartoonSet cartoonSet=super.get(cartoonSetId);
			cartoonSet.setPlayCount(cartoonSet.getPlayCount()+1);
			cartoonSet.setBuyCount(cartoonSet.getBuyCount()+1);
			flag=true;
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
