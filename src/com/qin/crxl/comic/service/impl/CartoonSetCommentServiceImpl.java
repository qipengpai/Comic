...
//	@Cacheable(value="CartoonSetComment",key="#root.methodName.concat(#cartoonId).concat(#cartoonSetId)")
	public int getAllCartoonSetCommentCount(String cartoonId, String cartoonSetId) {
		// 查看该话评论人数
		int num = super
				.gettotalpage("CSC-"+cartoonSetId+"-Num",3600,"SELECT COUNT(*) FROM  CartoonSetComment WHERE  cartoonId='"
						+ cartoonId
						+"'  AND cartoonSetId='"
								+ cartoonSetId
...
//	@Cacheable(value="CartoonSetComment",key="#cartoonId.concat(#cartoonSetId).concat(#bestNew).concat(#nowPage)")
	public List<CartoonSetComment> getALLCartoonSetComment(
		...
		List<CartoonSetComment> list = super.SQL("CSC-"+cartoonSetId+"-"+bestNew+"-"+nowPage,3600,sb.toString(),
...
//	@Cacheable(value="CartoonSetCommentSon",key="#root.methodName.concat(#id)")
	public int getCartoonSetComment2Count(String cartoonSetId,String id) {
		// 查看该话评论的评论数
		int num = super
				.gettotalpage("CSCS-" + cartoonSetId + "-" + id + "-Num",3600,"SELECT COUNT(*) FROM  CartoonSetComment WHERE commentId='"
						+ id
						+ "' AND deleteCartoonSetComment=1 ");
		return num;
	}
...
//	@CacheEvict(value="CartoonSetComment",allEntries=true)
	public boolean addCartoonSetComment(
		...
		try {
			 ...
			 */
			JedisUtil.batchDel("CSC-" + cartoonSetCommentData
					.getCartoonSetId());
			flag = true;
		} catch (Exception e) {
...
//	@Cacheable(value="CartoonSetCommentSon",key="#root.methodName.concat(#id).concat(#nowPage)")
	public List<CartoonSetComment> getALLCartoonSetCommentByCommentId(String cartoonSetId,String id,
			String nowPage) {
		...
		List<CartoonSetComment> list = super.SQL("CSCS-" + cartoonSetId + "-" + id
				+ "-" + nowPage, 3600,sb.toString(),
				CartoonSetComment.class);
		return list;
	}
...
//	@CacheEvict(value="CartoonSetCommentSon",allEntries=true)
	public boolean addCartoonCommentSetConmment(
			CartoonSetCommentData cartoonSetCommentData, String userId,String cartoonSetId) {
		// 评论该评论
		boolean flag = false;
		try {
			// int num =
	...
	@Override
//	@Cacheable(value="CartoonSetCommentSon",key="#root.methodName.concat(#id)")
	public List<CartoonSetComment> getCartoonSetCommentTwo(String cartoonsetId,String id) {
		// 查看评论的子评论
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT  * FROM  CartoonSetComment  WHERE  1=1 AND deleteCartoonSetComment=1 AND commentId='"
				+ id + "' ORDER BY commentDate ASC LIMIT 0,2");
		List<CartoonSetComment> list = super.SQL("CSCS-" + cartoonsetId + "-" + id
				+ "-Two", 3600,sb.toString(),
				CartoonSetComment.class);
		return list;
	}
