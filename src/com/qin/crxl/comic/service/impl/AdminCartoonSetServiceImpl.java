...
//			cartoonSet.setWatchState(Integer.parseInt(cartoonSetData.getWatchState()));//观看状态
	...
	@Override
//	@CacheEvict(value="CartoonSet",allEntries=true)
	public boolean deleteCartoonSet(CartoonSetData cartoonSetData) {
		try {
			...
			if(ParaClick.clickObj(cartoonSet)){
				...
				/////////////redis
				JedisUtil.batchDel("MHPhoto"+cartoonSetData.getCartoonId()+cartoonSetData.getId());
				@SuppressWarnings("unchecked")
				...
				/////////////redis
				JedisUtil.batchDel("CVeryOk-"+cartoonSetData.getId());
				JedisUtil.batchDel("Set-"+cartoonSetData.getCartoonId());
				return true;
			}else{
				return false;
			}
...
//	@CacheEvict(value="CartoonSet",allEntries=true)
				...
				if(!ParaClick.clickString(cartoonSetData.getWatchState())){
					...
					if("1".equals(cartoonSetData.getWatchState())){//漫画话上架
						...
						if(!"1".equals(cartoonSetData.getSort().trim())){
							boolean flag=adminCartoonService.updateCartoonTitle(nextTitle.get(0),cartoonSetData.getCartoonId());
							if (!flag) {
								return false;
							}
	...
	public boolean updateAllCartoonSetState(CartoonSetData cartoonSetData) {
		...
		try {
			List<CartoonSet> sql = SQL("select * from CartoonSet where cartoonId='"+cartoonSetData.getCartoonId()+"' and sort="+cartoonSetData.getSort(),CartoonSet.class);
			...
			SQL("update Cartoon set updateDate='"+DateUtil.getdate()+"',updateTile='"+sql.get(0).getTitile()+"' where id='"+cartoonSetData.getCartoonId()+"'");
			/////////redis
			JedisUtil.batchDel("Set-"+cartoonSetData.getCartoonId());
			JedisUtil.batchDel("Comic");
			return true;
