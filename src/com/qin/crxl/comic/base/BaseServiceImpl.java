package com.qin.crxl.comic.base;


import java.io.Serializable;
import java.lang.reflect.Type;
import java.lang.reflect.ParameterizedType;
import java.util.ArrayList;
import java.util.List;

import javax.annotation.Resource;

import org.hibernate.Session;
import org.springframework.stereotype.Service;

import com.qin.crxl.comic.tool.ParaClick;
import com.qin.game.entity.redis.JedisUtil;



/**
 * Service层基类实现类。该类是一个抽象类。
 * @author Xiongyan
 * @date 2015年5月6日 下午6:59:53
 * @version 0.0.1
 * @param <T>
 */
@Service
public abstract class BaseServiceImpl<T> implements BaseService<T> {
	@Resource
	private BaseDao<T> baseDao;

    private Class<T> clazz;

    @SuppressWarnings({ "unchecked", "rawtypes" })
    public BaseServiceImpl() {
        // 子类
        Class cla = getClass();
        // 通过子类获取到父类 
        // 泛型参数
        Type type = cla.getGenericSuperclass();
        if (type instanceof ParameterizedType) {
            ParameterizedType pType = (ParameterizedType) type;
            clazz = (Class<T>) pType.getActualTypeArguments()[0];
        }
    }

    /*
     * 保存数据
     * @author Xiongyan
     * @date 2015年5月6日 下午6:55:23
     * (non-Javadoc)
     * @throws 
     * @see cn.mylife.service.user.BaseService#save(java.lang.Object)
     */
    @Override
    public void save(T t) {
        baseDao.save(t);
    }

    /*
     * 根据ID删除数据
     * @author Xiongyan
     * @date 2015年5月6日 下午6:56:01
     * (non-Javadoc)
     * @throws 
     * @see cn.mylife.service.user.BaseService#delete(java.io.Serializable)
     */
    @Override
    public void delete(Serializable id) {
        baseDao.delete(id, clazz);
    }
    @Override
    public void delete(String redis,Serializable id) {
    	if(JedisUtil.exists(redis)){
    		baseDao.delete(id, clazz); 
    		JedisUtil.del(redis);
    	}else{
    		baseDao.delete(id, clazz);
    	}
    }
 
    @Override
    public void delete(T t) {
        baseDao.delete(t);
    }

    /*
     * 更新数据
     * @author Xiongyan
     * @date 2015年5月6日 下午6:56:51
     * (non-Javadoc)
     * @throws 
     * @see cn.mylife.service.user.BaseService#update(java.lang.Object)
     */
    @Override
    public void update(T t) {
        baseDao.update(t);
    }

    /*
     * 根据ID获得数据
     * @author Xiongyan
     * @date 2015年5月6日 下午6:57:54
     * (non-Javadoc)
     * @throws 
     * @see cn.mylife.service.user.BaseService#get(java.io.Serializable)
     */
    @Override
    public T get(Serializable id) {
       T t=  baseDao.get(id, clazz);
        return t;
    }

    /*
     * 获取所有的数据
     * @author Xiongyan
     * @date 2015年5月6日 下午6:58:34
     * (non-Javadoc)
     * @throws 
     * @see cn.mylife.service.user.BaseService#getAll()
     */
    @Override
    public List<T> getAll() {
    	List<T> t=baseDao.getAll(clazz);
        return t;
    }
    
    @Override
    public List<T> getAll(String redis,int outTile){
    	if(JedisUtil.exists(redis)){
    		return (List<T>) JedisUtil.get(redis);
    	}else{
    		List<T> t=baseDao.getAll(clazz);
    		JedisUtil.setExpire("redis", t, outTile);
    		return t;
    	}
    }

	@Override
	public List<T> SQL(String sql,Class<T> entity) {
		// TODO Auto-generated method stub
			System.err.println(sql);
			List<T> t=baseDao.SQL(sql,entity);
			return t;
	}

	
	@SuppressWarnings("unchecked")
	@Override
	public List<T> SQL(String redis,int outTime,String sql,Class<T> entity) {
		if("".equals(redis)){
			redis=sql;
		}
		System.err.println(sql);
		if(JedisUtil.exists(redis)){
			return (List<T>) JedisUtil.get(redis);
		}else{
			List<T> t=baseDao.SQL(sql,entity);
			JedisUtil.setExpire(redis, t, outTime);
			return t;
		}
		
			
	}

	
	@Override
	public List<T> HQL(String hql) {
		// TODO Auto-generated method stub
		return baseDao.HQL(hql);
	}
	
	@SuppressWarnings("unchecked")
	@Override
	public List<T> HQL(String redis,int outTime,String hql) {
		// TODO Auto-generated method stub
		if("".equals(redis)){
			redis=hql;
		}
		if(JedisUtil.exists(redis)){
			return (List<T>) JedisUtil.get(redis);
		}else{
			List<T> t=baseDao.HQL(hql);
			JedisUtil.setExpire(redis, t,outTime);
			return t;
		}
	}

	@Override
	public int gettotalpage(String sql) {
		// TODO Auto-generated method stub
		return baseDao.gettotalpage(sql);
	}
	
	@Override
	public int gettotalpage(String redis,int outTime,String sql) {
		// TODO Auto-generated method stub
		if("".equals(redis)){
			redis=sql;
		}
		if(JedisUtil.exists(redis)){
			return (int) JedisUtil.get(redis);
		}else{
			int num=baseDao.gettotalpage(sql);
			if(num>=1){
				JedisUtil.setExpire(redis, num, outTime);
			}
			return num;
		}
	}
	
	
	
	@Override
	public Double getSUM(String sql) {
		// TODO Auto-generated method stub
		return baseDao.getSUM(sql);
	}
	
	@Override
	public Double getSUM(String redis,int outTime,String sql){
		if("".equals(redis)){
			redis=sql;
		}
		if(JedisUtil.exists(redis)){
			return  (Double) JedisUtil.get(redis);
		}else{
			Double num=baseDao.getSUM(sql);
			if(num!=null){
				JedisUtil.setExpire(redis, num, outTime);
			}
			return num;
		}
	}
	/**
	 * 执行非查询sql 语句
	 */
	@Override
	public void SQL(String sql) throws Exception {
		baseDao.SQL(sql);
	}
	
	
	protected  Session getSessionFactory(){
		return baseDao.getSession();
	};
	
}