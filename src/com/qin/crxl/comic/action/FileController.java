...
public class FileController {
	 ...
	 * */
	@ResponseBody
	@RequestMapping(value = ActionUrl.USER_UPLOD_PORTRACT_WATER, method = RequestMethod.POST)
	public Model userUplod3(@RequestParam(value = "file") MultipartFile[] file)
			throws Exception {
		List<Object> list = new ArrayList<Object>();
		try {
			UploadDemo uploadDemo = new UploadDemo();
			for (int i = 0; i < file.length; i++) {
				String userfile = uploadDemo.upload2(file[i]);
				if (ParaClick.clickString(userfile)) {
					return new Model(500, "上传失败");
				}
				BufferedImage image = ImageIO.read(file[i].getInputStream());
