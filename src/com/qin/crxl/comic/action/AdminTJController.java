...
public class AdminTJController {
	...
	public Model selectCartoonkaKaMoney(UserEntityData userEntityData) {
		...
		for (int i = 0; i < cartoonMoneyWaterLie.size(); i++) {
			cartoonMoneyWaterLie.get(i)[2] = df.format(Double.parseDouble(String
	...
	public Model selectCartoonProfit(AdminFansData adminFansData) {
		...
		List<Object[]> profit = adminUserOrderService.selectCartoonOrderProfit(
				adminFansData.getStarTime(), adminFansData.getEndTime());
		...
		for (int i = 0; i < profit.size(); i++) {
			profit.get(i)[1] = df.format(Double.parseDouble(String
					.valueOf(profit.get(i)[1])));
			AdminFansData adminFans = new AdminFansData();
			if (ParaClick.clickString(String.valueOf(profit.get(i)[0]))
					|| String.valueOf(profit.get(i)[0]).equals("null")) {
				adminFans.setName("平台用户");
			} else {
				adminFans.setName(String.valueOf(profit.get(i)[0]));
			}
			adminFans
					.setValue(Double.parseDouble(String.valueOf(profit.get(i)[1])));
			list.add(adminFans);
			allMoney += Double.parseDouble(String.valueOf(profit.get(i)[1]));
		}
		Model model = new Model();
		model.setError(200);
		model.setMsg("各个公众号渠道的咔咔豆收益");
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = ActionUrl.EXPORT_EXCEL, method = RequestMethod.GET)
	public Model exportExcel(HttpServletResponse response,HttpServletRequest request,AdminFansData adminFansData) {
	if (ParaClick.clickString(adminFansData.getStarTime())
				&& ParaClick.clickString(adminFansData.getEndTime())) {
			adminFansData.setStarTime(DateUtil.getdate_yyyy_MM_dd());
			adminFansData.setEndTime(DateUtil.getdate_yyyy_MM_dd());
		...
		try {
			...
			response.setHeader("Content-disposition", "attachment; filename="
					+ fileName + ".xls");
			String[] titles = { "渠道", "收益" };
			adminUserOrderService.export(titles, out,adminFansData);
