...
public class CartoonSetController {
	 ...
	 * @ResponseBody
	 * 
	 * @RequestMapping(value = ActionUrl.GET_CARTOONSET_BYID, method =
	 * RequestMethod.POST) public Model getCartoonSetById(CartoonData
	 * cartoonData) throws Exception { UserEntity userEntity=null; if
	 * (!ParaClick.clickString(cartoonData.getUuid())) {
	 * cartoonData.clickUser(); userEntity =
	 * userService.getUserInfoByAPP(cartoonData
	 * .getUserId(),cartoonData.getUuid()); if (userEntity == null) { return new
	 * Model(700, "无用户"); } }else{ cartoonData.clickUser(); userEntity =
	 * userService.getUserInfoById(cartoonData .getUserId()); if (userEntity ==
	 * null) { return new Model(404, "无用户"); } } // if
	 * (ParaClick.clickString(cartoonData.getNowPage())) { //
	 * cartoonData.setNowPage("1"); // } Model model = new Model(); // int num =
	 * cartoonSetService.getPageNum(cartoonData); // if (num == 0) { // return
	 * new Model(500, "未查询到信息"); // } List<Object> list2 = new
	 * ArrayList<Object>(); List<CartoonSet> cartoonSet = cartoonSetService
	 * .getByCartoonId(cartoonData); if (!ParaClick.clickList(cartoonSet)) {
	 * return new Model(500, "查询话集异常"); } String mode = ""; if
	 * (Integer.parseInt(cartoonData.getMode()) == 0) { mode = "1"; } else {
	 * mode = "0"; } List<HistoryRecord> historyRecord = historyRecordService
	 * .getWatchStateByUserAndCartoonId(cartoonData.getId(),
	 * userEntity.getUserId()); for (CartoonSet cartoonSet2 : cartoonSet) {
	 * Hashtable<String, Object> map = new Hashtable<String, Object>(); //
	 * List<VeryOk> list = veryOkService.getUserVseryOk(cartoonSet2 // .getId(),
	 * userEntity); // if (!ParaClick.clickList(list)) { // map.put("veryOk",
	 * 0); // } else { // map.put("veryOk", 1); // } if
	 * (ParaClick.clickList(historyRecord)) { if
	 * (historyRecord.get(0).getWatchState() == 2) { if
	 * (historyRecord.get(0).getCartoonSetId() .equals(cartoonSet2.getId())) {
	 * map.put("watchState", 2); } else { if (cartoonSet2.getMoneyState() == 1)
	 * { MallCartoonOrder mallCartoonOrder = mallCartoonOrderService
	 * .getHistory(cartoonSet2.getCartoonId(), cartoonSet2.getId(),
	 * userEntity.getUserId()); if (mallCartoonOrder == null) {
	 * map.put("watchState", 0); } else { map.put("watchState", 1); } } else {
	 * map.put("watchState", 1); } } } else { if (cartoonSet2.getMoneyState() ==
	 * 1) { MallCartoonOrder mallCartoonOrder = mallCartoonOrderService
	 * .getHistory(cartoonSet2.getCartoonId(), cartoonSet2.getId(),
	 * userEntity.getUserId()); if (mallCartoonOrder == null) {
	 * map.put("watchState", 0); } else { map.put("watchState", 1); } } else {
	 * map.put("watchState", 1); } } } else { if (cartoonSet2.getMoneyState() ==
	 * 1) { map.put("watchState", 0); } else { map.put("watchState", 1); } }
	 * CartoonSetNew cartoonSetNew = new CartoonSetNew(); if
	 * (cartoonSet2.getUpdateDate().indexOf(" ") != -1) {
	 * cartoonSetNew.setUpdateDate(cartoonSet2.getUpdateDate() .substring(0,
	 * cartoonSet2.getUpdateDate().lastIndexOf(" "))); } if
	 * (cartoonSet2.getPlayCount() < 10000) {
	 * cartoonSetNew.setPlayCount(cartoonSet2.getPlayCount() + ""); } else {
	 * double n = (double) cartoonSet2.getPlayCount() / 10000; String num2 = n +
	 * ""; cartoonSetNew.setPlayCount(num2.substring(0, num2.indexOf(".", 0) +
	 * 2) + "万"); } if (cartoonSet2.getOkCount() < 10000) {
	 * cartoonSetNew.setOkCount(cartoonSet2.getOkCount() + ""); } else { double
	 * n = (double) cartoonSet2.getOkCount() / 10000; String num2 = n + "";
	 * cartoonSetNew.setOkCount(num2.substring(0, num2.indexOf(".", 0) + 2) +
	 * "万"); } cartoonSetNew.setBarrageCount(cartoonSet2.getBarrageCount());
	 ...
	 * cartoonSet.get(cartoonSet.size()).getSort()); }else{ map.put("min",
	 * cartoonSet.get(0).getSort()); map.put("max",
	 * cartoonSet.get(cartoonSet.size()).getSort()); }
	 * 
	 * list2.add(map); } model.setError(200); //
	 * model.setNowpage(Integer.parseInt(cartoonData.getNowPage())); //
	 * model.setTotalpage((num + 10 - 1) / 10); model.setObj(list2);
	 * model.setSpare(mode); return model; }
	 */
	...
	public Model getCartoonSetById(CartoonData cartoonData) throws Exception {
		...
		if (!ParaClick.clickString(cartoonData.getUuid())) {
			...
			userEntity = userService.getUserInfoByAPP(cartoonData.getUserId(),
					cartoonData.getUuid());
			if (userEntity == null) {
			...
			cartoonData.clickUser();
			userEntity = userService.get(cartoonData.getUserId());
			if (userEntity == null) {
		...
		if (userEntity.getVipId() == 2) {
			...
			List<MallCartoonOrder> mallCartoonOrder2 = mallCartoonOrderService
					.getByUser(cartoonData.getId(), userEntity.getUserId());
			for (CartoonSet cartoonSet2 : cartoonSet) {
				Hashtable<String, Object> map = new Hashtable<String, Object>();
				CartoonSetNew cartoonSetNew = new CartoonSetNew();
				if (cartoonSet2.getMoneyState() == 1) {
					int flag = 0;
					for (MallCartoonOrder mall : mallCartoonOrder2) {
						if (mall.getCartoonSetId().equals(cartoonSet2.getId())) {
							flag = 1;
							break;
						}
					}
					map.put("watchState", flag);
				} else {
					map.put("watchState", 1);
				}
				if (cartoonSet2.getUpdateDate().indexOf(" ") != -1) {
