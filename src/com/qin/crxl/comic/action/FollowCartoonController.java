...
public class FollowCartoonController {
	...
	public Model getCartoonInfoById(CartoonData cartoonData) throws Exception {
		...
		if(Integer.parseInt(cartoonData.getFollowState())==1){
			boolean flag = followCartoonService.deleteFollowCartoon(cartoonData.getId(),
					userEntity.getUserId());
			if (!flag) {
				return new Model(500, "取消关注失败 ");
			}
		}else{
			boolean flag = followCartoonService.followCartoon(cartoonData.getId(),
					userEntity.getUserId());
			if (!flag) {
		...
		List<FollowCartoon> followCartoon = followCartoonService.getFollowByid(
				cartoonData.getId(), userEntity.getUserId());
...
//			}
	 ...
	 * */
	@ResponseBody
	@RequestMapping(value = ActionUrl.IOS_GET_MY_FOLLOW_CARTOON_NEWS, method = RequestMethod.POST)
	public Model getCartoonByBookshelfNewsByIos(CartoonData cartoonData)
			throws Exception {
		cartoonData.clickUser();
		UserEntity userEntity = userService.getUserInfoById(cartoonData
				.getUserId());
		...
		String newTime = DateUtil.dateToTimeStamp(new Date());
		Model model = new Model();
		List<FollowCartoon> list = followCartoonService.getCartoonByLoginByIos(userEntity);
		if (!ParaClick.clickList(list)) {
			return new Model(200, "暫無收藏");
		}
		List<Cartoon> list232=new ArrayList<Cartoon>();
...
//			}
	 ...
	 * */
	@ResponseBody
	@RequestMapping(value = ActionUrl.IOS_GET_MY_FOLLOW_CARTOON, method = RequestMethod.POST)
	public Model getCartoonByBookshelfByIos(CartoonData cartoonData)
			throws Exception {
		...
		Model model = new Model();
		int num = followCartoonService.getCartoonByBookshelfCountByIos(cartoonData,
				userEntity);
		if (num == 0) {
			return new Model(200, "暫無收藏");
		}
		List<FollowCartoon> list = followCartoonService.getCartoonByBookshelfByIos(
				cartoonData, userEntity);
		if (!ParaClick.clickList(list)) {
			return new Model(200, "暫無收藏");
		}
